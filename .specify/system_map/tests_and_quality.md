...
**Governing Principles**: Constitution Principle IV (TDD & Zero Zombie Tests)

---

## 1. Test Suite Registry (`tests/`)

The active test suite comprises **76 tests** with 100% pass rate and zero third-party warnings.

| Test File | Target Layer | Scope & Responsibilities |
|---|---|---|
| [`tests/unit/test_architecture_contracts.py`](file:///E:/JE/tests/unit/test_architecture_contracts.py) | **Architecture Linter** | AST-level verification: strictly forbids domain models from importing services/adapters (DIP), ensures retired files do not exist (YAGNI), and verifies deleted RPC endpoints are gone. |
| [`tests/unit/test_frontend_contracts.py`](file:///E:/JE/tests/unit/test_frontend_contracts.py) | **Frontend Contract** | Verifies `<script>` tags in `index.html` exist on disk, DOM IDs called in JS exist in HTML, I18n methods exist, and bilingual translation key parity (`uk` == `en`). |
